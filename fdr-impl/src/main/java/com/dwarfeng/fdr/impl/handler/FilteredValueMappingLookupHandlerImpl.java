...
@Component
public class FilteredValueMappingLookupHandlerImpl implements FilteredValueMappingLookupHandler {

    private static final Logger LOGGER = LoggerFactory.getLogger(FilteredValueMappingLookupHandlerImpl.class);

    private final ApplicationContext ctx;

    private final FilteredValueMaintainService maintainService;
    ...
    @Value("${mapping_lookup.filtered_value.finished_session_min_retain_time}")
    private long finishedSessionMinRetainTime;
    @Value("${mapping_lookup.filtered_value.max_lookup_page_size}")
    private int maxLookupPageSize;
    @Value("${mapping_lookup.filtered_value.max_lookup_period_span}")
    ...
    public void init() throws Exception {
        FilteredValueMappingLookupHandlerImpl.CleanupTask cleanupTask = ctx.getBean(CleanupTask.class, this);
    ...
    class LookupTask implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                List<FilteredValue> singleLookupResult;
                    ...
                    do {
                        ...
                        if (notLastPeriodFlag) {
                            singleLookupResult = maintainService.lookupAsList(
                                    FilteredValueMaintainService.CHILD_FOR_POINT_BETWEEN_RB_OPEN,
                ...
                FilteredValue rawPrevious = maintainService.lookupFirst(
                        FilteredValueMaintainService.CHILD_FOR_POINT_PREVIOUS, new Object[]{pointKey, startDate}
                );
                TimedValue previous = Optional.ofNullable(rawPrevious).map(
                        raw -> new TimedValue(raw.getValue(), raw.getHappenedDate())
                ).orElse(null);
                FilteredValue rawRear = maintainService.lookupFirst(
                        FilteredValueMaintainService.CHILD_FOR_POINT_REAR, new Object[]{pointKey, endDate}
                );
    ...
    class CleanupTask implements Runnable {
        ...
        @Override
        public void run() {
            LOGGER.debug("被过滤值映射查询处理器: 清理计划定期执行...");
            internalCleanup();
        }
    }
}
