...
@Component
public class TriggeredValueMappingLookupHandlerImpl implements TriggeredValueMappingLookupHandler {

    private static final Logger LOGGER = LoggerFactory.getLogger(TriggeredValueMappingLookupHandlerImpl.class);

    private final ApplicationContext ctx;

    private final TriggeredValueMaintainService maintainService;
    ...
    @Value("${mapping_lookup.triggered_value.cleanup_task_cron}")
    private String cleanupTaskCron;
    @Value("${mapping_lookup.triggered_value.finished_session_min_retain_time}")
    private long finishedSessionMinRetainTime;
    @Value("${mapping_lookup.triggered_value.max_lookup_page_size}")
    private int maxLookupPageSize;
    @Value("${mapping_lookup.triggered_value.max_lookup_period_span}")
    ...
    public TriggeredValueMappingLookupHandlerImpl(
            ApplicationContext ctx, TriggeredValueMaintainService maintainService,
        ...
        public void run() {
            ...
            try {
                    ...
                    do {
                        ...
                        if (notLastPeriodFlag) {
                            singleLookupResult = maintainService.lookupAsList(
                                    TriggeredValueMaintainService.CHILD_FOR_POINT_BETWEEN_RB_OPEN,
                ...
                String logMessage = String.format(
                        "被触发值映射查询发生异常, 获取会话结果时将抛出此异常, 会话主键: %s, 异常信息: ", session.getKey()
                );
    ...
    class CleanupTask implements Runnable {
        ...
        @Override
        public void run() {
            LOGGER.debug("被触发值映射查询处理器: 清理计划定期执行...");
            internalCleanup();
        }
    }
}
