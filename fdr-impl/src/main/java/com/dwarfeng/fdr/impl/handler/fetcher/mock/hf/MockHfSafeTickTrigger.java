 ...
 * @since 3.1.0
 */
public final class MockHfSafeTickTrigger implements Trigger {

    private static final Logger LOGGER = LoggerFactory.getLogger(MockHfSafeTickTrigger.class);

    private final Trigger delegateTrigger;
    private final long misfireThreshold;
    private final long misfirePostpone;

    public MockHfSafeTickTrigger(Trigger delegateTrigger, long misfireThreshold, long misfirePostpone) {
    ...
    public Date nextExecutionTime(@NonNull TriggerContext triggerContext) {
        ...
        LOGGER.warn(
                "高频模拟任务延迟超过阈值, 原计划触发时间戳为 {}, 当前时间戳为 {}, 延迟时长为 {}, 超过阈值 {}, " +
