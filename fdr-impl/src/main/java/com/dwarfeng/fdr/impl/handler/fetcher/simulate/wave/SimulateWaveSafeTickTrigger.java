 ...
 * @since 3.1.0
 */
public final class SimulateWaveSafeTickTrigger implements Trigger {

    private static final Logger LOGGER = LoggerFactory.getLogger(SimulateWaveSafeTickTrigger.class);

    private final Trigger delegateTrigger;
    private final long misfireThreshold;
    private final long misfirePostpone;

    public SimulateWaveSafeTickTrigger(Trigger delegateTrigger, long misfireThreshold, long misfirePostpone) {
    ...
    public Date nextExecutionTime(@NonNull TriggerContext triggerContext) {
        ...
        LOGGER.warn(
                "函数波形模拟任务延迟超过阈值, 原计划触发时间戳为 {}, 当前时间戳为 {}, 延迟时长为 {}, 超过阈值 {}, " +
