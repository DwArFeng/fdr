 ...
 * @since 2.0.0
 */
public abstract class WatchCommand<D extends Data> extends CliCommand {
    ...
    protected final WatchQosService<D> watchQosService;

    public WatchCommand(
            String identity, String description, WatchQosService<D> watchQosService
    ) {
        super(identity, description, cmdLineSyntax(identity));
        this.watchQosService = watchQosService;
    }

    @Override
    protected List<Option> buildOptions() {
        List<Option> list = new ArrayList<>();
        list.add(Option.builder(COMMAND_OPTION_INSPECT).desc("查询指令").build());
    ...
    private void handleInspect(Context context, CommandLine cmd) throws Exception {
        ...
        // 查询数据。
        List<D> datas = watchQosService.inspect(pointKeys);
    ...
    private void handleQuery(Context context, CommandLine cmd) throws Exception {
        ...
        // 查询数据。
        QueryResult<D> queryResult = watchQosService.query(queryInfo);
        List<D> datas = queryResult.getDatas();
    ...
    private void handleLookup(Context context, CommandLine cmd) throws Exception {
        ...
        // 查询数据。
        LookupResult lookupResult = watchQosService.lookup(lookupInfo);
        List<LookupResult.Sequence> sequences = lookupResult.getSequences();
        ...
        while (true) {
            ...
            LookupResult.Sequence sequence = sequences.get(sequenceIndex);
            ...
            List<LookupResult.Item> items = sequence.getItems();
            ...
            while (true) {
                ...
                context.sendMessage("");
                for (int i = cropResult.beginIndex; i < cropResult.endIndex; i++) {
                    LookupResult.Item item = items.get(i);
    ...
    protected abstract void printLookupData(int i, int endIndex, LookupResult.Item item, Context context)
            throws Exception;
