...
FDR 是一款开箱即用的数据收集解决方案。

FDR 运行后，您可以通过程序提供的操作接口维护数据点，定义数据点的存储策略、清洗策略、过滤策略、触发策略，随后上线数据记录功能。

FDR 数据记录功能上线后，能从多种数据源中收集数据、处理数据、记录数据，并提供数据查询接口。
...
> 黑匣子可以用来记录飞机在飞行过程中的各种参数，如飞行时间、速度、高度、飞机倾斜度、发动机转速及温度等，
> 以及驾驶员与乘务人员和各个塔台之间的对话等。
> 当飞机发生故障或事故时，找到黑匣子并读出记录数据，能够帮助技术人员分析故障或失事原因。

真实的 FDR 长这样：

![真实的 FDR](docs/wiki/zh-CN/images/RealFdr.jpeg "真实的 FDR")
...
- 数据时间模型支持 `happenedDate + happenedDateNanoOffset` 组合表达，在保持毫秒级主时间轴兼容的同时支持同毫秒内更高精度排序与查询。
- 数据点采集策略高度可配置，支持自定义清洗策略、过滤策略、触发策略，满足多样化数据采集需求。
- 分布式架构，可部署多个节点，满足不同规模的数据采集需求。
- 多数据源支持，可以从多种途径获取数据。
- 数据值支持多种类型，如：整型、浮点型、布尔型、字符串等。
- 高性能架构，数据分析与数据记录均采用多线程模型，单节点处理能力强。
- 采用桥接器模式记录数据，支持多种数据存储方式，满足不同数据存储需求。
- 提供系统事件，可通过第三方组件监听系统事件并扩展功能。
- 提供数据查询接口，可直接查询数据点实时值与历史值。
- 提供数据映射查询接口，可对一段时间内的数据进行开窗查看、占空比查询、高通查询、低通查询等。
- 提供基于 `telnet` 的运维平台，可通过 `telnet` 连接并设置关键参数、查看系统状态。
